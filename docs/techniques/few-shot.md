...
先看你最可能见过的那个——OpenAI 官方文档开篇用的经典 few-shot 例子。它一次把「**指令里规定输出格式**」和「**给 1 个范例**」两件事都演给你看，特别适合建立直觉：
...
- **指令里写清了「所需格式」**：`地点: <列表>`——模型知道该吐什么结构。
- **给了 1 个「输入 → 输出」范例**：模型照着这个模板，把新文本里的地名（里斯本、香帕利莫德中心）抽出来，套进同样的「地点: …」格式。
- **范例放任务说明之后、待解输入之前**：顺序对，模型先学样式再干活。

::: warning 原版里有个真实小瑕疵，正好当反面教材
注意上面格式占位写的是 `<逗号分隔的**公司名称**列表>`，但任务明明是「提取**地名**」，实际输出也是地名（里斯本是城市、香帕利莫德中心是研究机构）。

**这就是 few-shot 的常见坑：占位/说明文字要和真实任务对齐。** 写成"公司名称"纯属原版笔误，模型靠上下文勉强猜对了；但你要是照抄这种"说明与任务不符"的模板，遇到边界 case 它就容易懵。写 few-shot 时，格式说明、范例、真实任务三者必须一致。
:::

理解了上面这个「纯文本」版，再看下面两个「能直接跑」的代码版，思路完全一样，只是套了 SDK 调用外壳。
...
- [ ] 会挑 1~N 个代表性、格式一致的样例
- [ ] 明白样例放「任务说明之后、新输入之前」
- [ ] 见过"提取地名"这个经典 few-shot 范例，理解"格式说明要对齐任务"
- [ ] 会和零-shot 做取舍，不滥用
